...
function App() {
  ...
  const [itinerary, setItinerary] = useState<TravelPlannerFinalOutput | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await travelPlannerApi({
      location: from,
      destination,
      duration,
      ...
      {isLoading ? (
        ...
        <div>
            ...
            <>
              <header className="hero-panel">
                <div>
                  ...
                  <h1>{itinerary.travelPlannerOutput.title}</h1>
                  <p className="hero-copy">
                    Discover a polished, day-by-day journey from {fromToShown}{" "}
                    to {destinationToShown} with sightseeing, trains, and
                    culture built into a clear travel timeline.
                  </p>
                </div>
              </header>
            </>
          ...
          <main className="timeline-grid">
            {itinerary &&
              itinerary.travelPlannerOutput.itinerary.map((entry, index) => (
          ...
          <div>
            {itinerary?.travelPlannerOutput?.budget && <Budget budget={itinerary?.travelPlannerOutput?.budget} />}
          </div>
